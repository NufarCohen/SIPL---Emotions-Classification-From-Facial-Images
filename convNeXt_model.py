...
def load_data(
    ...
    """
    # ImageNet stats (ConvNeXt pretrained uses these)
    mean = (0.485, 0.456, 0.406)
    std  = (0.229, 0.224, 0.225)
    img_size = 384 if 'xlarge' in str(variant).lower() else 224
    ...
    if mode == 'rgb224':
        ...
        raise ValueError("mode must be 'rgb224'")
...
class ConvNeXtFER(nn.Module):
    ...
    """
    def __init__(self, num_classes=7, in_chans=3, pretrained=False, model_name="convnext_base"):
        super().__init__()
...
def train_convnext(
    ...
    with ema.average_parameters():
        te_loss, te_acc, te_f1, te_preds, te_labels = run_epoch(
            model, test_loader, loss_fn, optimizer=None, scaler=None, device=device, ema=None, train=False, return_preds=True
        )
...
def main():
    ...
    print(f"Model version is ConvNeXt ({variant}) with batch size {batch_size}\n")

    train_convnext(
        data_root=data_root,
        dataset_name="MyFER",
        num_classes=None,      # infer from folders
        variant=variant,
        mode="rgb224",
        epochs=30,
        base_lr=1e-4,
